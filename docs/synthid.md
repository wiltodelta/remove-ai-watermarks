...
redirected a signed-in browser to a Google 404. On 2026-09-23 the same entry
point redirected to `deepmind.google.com/frontiers/synthid`, which also returned
404 to a signed-in browser. OpenAI now documents a
...
watermarked, so Google AI C2PA establishes SynthID. Google Photos AI edits
are included on measurement rather than documentation. Google documents SynthID
in Photos only for Reimagine
([blog.google, 2025-08-20](https://blog.google/products-and-platforms/products/photos/ai-photo-editing-google-photos/)),
and a Photos manifest (signer common name `Google Photos`) records actions such
as `c2pa.deleted` or `c2pa.edited` with no description naming the tool and no
SynthID action. Yet Google's own checker in Gemini, given the four Photos AI
edits in [`data/captures/2026-09/google-photos/`](../data/captures/2026-09/google-photos/)
with their metadata stripped, found SynthID in all four on 2026-09-25: an Ask
...
[`data/captures/2026-09/google-photos-web/`](../data/captures/2026-09/google-photos-web/)).

What Google states, checked against each page on 2026-09-26; the step from these
statements to "every Photos AI edit" is this project's inference, not Google's:

- Reimagine ([blog.google, 2025-02-06](https://blog.google/feed/synthid-reimagine-magic-editor/)):
  Photos uses SynthID "to mark images edited with generative AI using Reimagine
  in Magic Editor", and "edits made using Reimagine may be too small for SynthID
  to label and detect".
- The Photos editor runs Nano Banana
  ([blog.google, 2025-11-11](https://blog.google/products-and-platforms/products/photos/nano-banana-ai-templates-ask-photos/)):
  "With Nano Banana in Photos' editor, you can now make all kinds of new
  transformations to your images", reached through "Help me edit". The post does
  not mention SynthID.
- The model marks its own output
  ([Google Developers Blog](https://developers.googleblog.com/en/introducing-gemini-2-5-flash-image/)):
  "All images created or edited with Gemini 2.5 Flash Image will include an
  invisible SynthID digital watermark".
- Photos calls Magic Eraser generative and signs non-AI edits only in a chain that
  already had credentials ([Photos Help](https://support.google.com/photos/answer/16496549)):
...
YouTube is the exception: it re-signs every upload with `YouTube Video
Processing Services`, so its manifest counts only when the chain carries a
SynthID action (measured 2026-09-24: a Gemini Omni Flash upload kept its
ingredient's SynthID action, an xAI Grok upload had none). OpenAI C2PA predates its
...
## 4. Adoption and current state (as of September 2026)

### 4.1 Google products

Google reports watermarking "over 100 billion images and videos and 60,000 years
of audio" ([Google, 2026-05-19](https://blog.google/innovation-and-ai/products/identifying-ai-generated-media-online/)).
The deployment split by surface matters for our tool:

| Surface                              | SynthID pixel | C2PA metadata | Visible sparkle |
|--------------------------------------|---------------|---------------|-----------------|
| Gemini app (generated images)        | YES           | YES (Google)  | YES             |
| Gemini API / AI Studio / Nano Banana | YES           | YES (Google) on all four image models, measured 2026-09-24: `gemini-2.5-flash-image`, `gemini-3.1-flash-image`, `gemini-3-pro-image`, `gemini-3.1-flash-lite-image` | NO on every measured API output |
| Vertex AI / Agent Platform models    | YES           | YES (Google, device "Google Media Processing Services") | Model-dependent |
| Gemini Omni video (Gemini app, Flow) | YES           | YES (Google)  | Optional Veo diamond |
| Gemini Omni Flash API (`gemini-omni-1.1-flash`) | YES, video with audio ([API docs](https://ai.google.dev/gemini-api/docs/omni)) | YES (Google), measured 2026-09-24; the docs do not mention it | NO on the measured output |

The visible mark is optional for every consumer tier since 2026-08-14: the
Gemini "Media watermark" setting switches it off for images, Omni video and
Lyria music, except on work accounts and in India, South Korea and Vietnam
without Ultra. The C2PA actions record the visible mark as its own
`c2pa.edited` action ("Added visible watermark"), next to "Added imperceptible
SynthID watermark" (measured on an Omni capture, 2026-09-23).

API captures on 2026-09-24 (one image each from Nano Banana, Nano Banana 2 and
Nano Banana Pro, three from Nano Banana 2 Lite, one Gemini Omni Flash video)
carried a valid Google C2PA manifest signed by `Google Media
Processing Services` with `c2pa.created` ("Created by Google Generative AI.")
and a SynthID `c2pa.edited` action, and no visible mark. The wording differs by
medium: the images say "Applied imperceptible SynthID watermark.", the video
"Added imperceptible SynthID watermark". The API video had no "Added visible
watermark" action. Its AAC audio track is covered by the same BMFF hash; whether
the audio carries its own SynthID payload is not visible locally. A Google manifest
without the visible-mark action therefore marks an export whose sparkle was
switched off, not a clean file.
...
- **ElevenLabs**: SynthID adopter for generated audio (same Google announcement).
  ElevenLabs runs its own Audio Detector; Gemini's verifier recognizes only
  Google content, so it is not an oracle for ElevenLabs audio.
- **OpenAI audio**: since 2026-07-31, audio generated through ChatGPT and the
  OpenAI API carries SynthID, verifiable in OpenAI's public tool and API
  ([OpenAI](https://openai.com/index/advancing-content-provenance/)). The API
  is `POST /v1/content_provenance_checks`: one file up to 50 MiB, audio of 60
  seconds or less after decoding in MP3, Opus, AAC, FLAC, WAV or PCM; audio
  results carry only a SynthID entry, with no C2PA
  ([guide](https://developers.openai.com/api/docs/guides/content-provenance)).
  The repository's OpenAI oracle adapter uploads PNG images only.
- **Apple** (announced, not shipped): Image Playground and Photos edits will get
  SynthID "in a software update later this year" ([Apple, 2026-09-14](https://www.apple.com/newsroom/2026/09/major-updates-for-apples-software-platforms-are-now-available/)).
  Measured iOS 27.0 output carries only an XMP credit and IPTC source type.
  Each adopter has its own verifier; a Google verdict says nothing about another
  vendor's payload.
