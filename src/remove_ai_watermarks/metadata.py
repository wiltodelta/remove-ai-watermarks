...
# Higgsfield stamps every PNG it serves with this ``tEXt`` chunk key holding the
# job UUID (measured 2026-09-24; see ``_huggingface_job_impl``). It marks the
# hosting job, not a specific model -- a medium-confidence AI signal.
...
# RIFF forms whose metadata chunks may follow the media data. C2PA puts its manifest
# in the LAST top-level chunk of every RIFF file (spec 2.4 section A.3.7), so a WAV
# or AVI longer than the head window hides it without this walk.
_RIFF_LATE_SCAN_FORMS: frozenset[bytes] = frozenset({b"WEBP", b"WAVE", b"AVI "})
...
def _scan_head_impl(image_path: Path, size: int) -> bytes:
    ...
    elif head[:4] == b"RIFF" and head[8:12] in _RIFF_LATE_SCAN_FORMS and len(head) == size:
...
def aigc_label_from_metadata(data: bytes, candidates: tuple[str, ...] = ()) -> dict[str, str] | None:
    ...
    for needle in ('"AIGC"', "AIGC{"):
        ...
        if result := _parse(text[brace:end], require_tc260_field=True):
            return result
    # Audio placements from TC260-PG-202510A (2025-08): a RIFF chunk with id ``AIGC``
    # (WAV; 4-byte length before the JSON), an ID3v2 ``TXXX`` frame whose
    # description is ``AIGC`` (MP3; NUL before the UTF-8 value), and a Vorbis
    # comment ``AIGC=`` (OGG, Opus, FLAC). The JSON must still carry a TC260 field.
    for match in _AUDIO_AIGC_PREFIX.finditer(text):
        brace = match.end()
        try:
            _, end = json.JSONDecoder().raw_decode(text, brace)
        except ValueError:
            continue
        if result := _parse(text[brace:end].encode("latin-1").decode("utf-8", "replace"), require_tc260_field=True):
            return result
    return None


_AUDIO_AIGC_PREFIX = re.compile(r"AIGC(?:=|\x00|[\x00-\xff]{4})(?=\{)")
...
def _huggingface_job_impl(image_path: Path) -> str | None:
    """Return the Higgsfield job id if the image carries an ``hf-job-id`` PNG
    text chunk, else None.

    Higgsfield (higgsfield.ai) writes an ``hf-job-id`` ``tEXt`` chunk holding the
    job UUID into the PNGs it serves, for its own Soul models and for hosted
    third-party models alike (23 of 26 PNG captures across 27 image models,
    2026-09-24; Soul v1, Soul Cinema and Wan 2.2 arrived without it). The name is
    kept from an earlier reading as Hugging Face: no Hugging Face source documents
    the chunk. It identifies the hosting job, not a model -- a medium-confidence
    AI signal, not proof of AI pixels on its own.
    """
    try:
        from PIL import Image
...
def synthid_source(image_path: Path, *, c2pa_info: dict[str, Any] | None = None) -> str | None:
    ...
    the mark, except for YouTube re-encodes, which need a recorded SynthID
    action. OpenAI C2PA existed before OpenAI adopted SynthID, so OpenAI also
...
def get_ai_metadata(image_path: Path) -> dict[str, str]:
    ...
    # Higgsfield job marker (hf-job-id PNG text chunk); key name kept for compatibility.
    if job := huggingface_job(image_path):
        result.setdefault("huggingface_job", f"Higgsfield job ({job})")
...
# GIF application extensions that carry playback behaviour, not provenance. Every
# other application extension (XMP ``XMP DataXMP``, C2PA ``C2PA_GIF``, vendor blobs)
# and every comment extension is dropped by the GIF walker.
_GIF_PLAYBACK_APP_IDS: tuple[bytes, ...] = (b"NETSCAPE2.0", b"ANIMEXTS1.0")
_GIF_ICC_APP_ID = b"ICCRGBG1012"


def _strip_gif_metadata_lossless(source_path: Path, output_path: Path, keep_standard: bool) -> bool:
    """Remove metadata extensions from a GIF by walking its blocks, frames untouched.

    Pillow has no GIF path that preserves every frame and palette bit for bit, and the
    generic re-save below would write PNG bytes under a ``.gif`` name and keep only the
    first frame (measured 2026-09-23 on a three-frame file). Image descriptors, local
    colour tables and LZW data are copied verbatim; graphic-control and plain-text
    extensions stay; application extensions survive only for looping and, when
    ``keep_standard``, the ICC profile. Returns False when the bytes are not a walkable
    GIF so the caller can fail safe.
    """
    data = source_path.read_bytes()
    ...
    try:
        ...
        def sub_blocks_end(start: int) -> int:
            cursor = start
            while True:
                size = data[cursor]
                cursor += 1 + size
        ...
        while pos < len(data):
            ...
            if introducer == 0x3B:  # trailer
                out += data[pos : pos + 1]
                break
            if introducer == 0x2C:  # image descriptor, optional local colour table, LZW data
                local_packed = data[pos + 9]
                table = 3 * (2 ** ((local_packed & 0x07) + 1)) if local_packed & 0x80 else 0
                end = sub_blocks_end(pos + 10 + table + 1)
                out += data[pos:end]
                pos = end
                continue
            if introducer != 0x21:
                return False
            label = data[pos + 1]
            end = sub_blocks_end(pos + 2)
            keep = label in (0xF9, 0x01)
            if label == 0xFF and data[pos + 2] == 11:
                app_id = bytes(data[pos + 3 : pos + 14])
                keep = app_id in _GIF_PLAYBACK_APP_IDS or (keep_standard and app_id == _GIF_ICC_APP_ID)
            if keep:
                out += data[pos:end]
            pos = end
        else:
...
def remove_ai_metadata(
    ...
    if head[:6] in (b"GIF87a", b"GIF89a"):
        if _strip_gif_metadata_lossless(source_path, output_path, keep_standard):
            return output_path
        logger.warning("Could not walk the blocks of GIF %s; copied through unchanged", source_path)
    ...
    if head[:4] in (b"II*\x00", b"MM\x00*"):
        # TIFF and DNG: re-saving through Pillow would write PNG bytes under the TIFF
        # name and flatten pages, bit depth and raw data. Refuse rather than damage it.
        raise ValueError(f"metadata removal does not support TIFF/DNG containers: {source_path.name}")
